...
class ImageController extends Controller
{
    ...
    public function uploadImage(Request $request)
    {
        ...
        if ($type != null && $type == 'xrt') {
            ...
            $tempName=uniqid().'.'.$request->file('image')->getClientOriginalExtension();
            Storage::disk('public')->putFileAs('./image',$request->file('image'),$tempName);
            /*
            if ($this->imageRepository->uploadImageToQiNiu($request, true))
                return back()->with('success', '上传成功');
            */
             return back()->with('success', $tempName);
